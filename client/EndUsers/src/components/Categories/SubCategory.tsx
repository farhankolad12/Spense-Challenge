...
export default function SubCategory() {
  ...
  const givenSubCategory = userData.subCategory.filter(
    (selectedCategory: SubCategoryType) => {
      return (
        selectedCategory.categoryId === givenCategory.id &&
  ...
  const innerSubCategory = userData.innerSubCategory.filter(
    (inner: InnerSubCategory) => {
      return (
        givenCategory.id === inner.categoryId &&
        givenSubCategory.id === inner.subCategoryId
      );
    }
  );
          ...
          <div className="card ">
            ...
            <div className="card-body">
              ...
              <ul style={{ listStyle: "square inside" }}>
                ...
                <ul style={{ listStyle: "square inside" }}>
                  {innerSubCategory.map(
                    (selectedInnerSubCategory: InnerSubCategory) => {
                      return (
                        <li key={selectedInnerSubCategory.id}>
                          <Link
                            className="text-dark"
                            to={`/categories/${category}/${subcategory}/${selectedInnerSubCategory.name}`}
                          >
                            {selectedInnerSubCategory.name}
                          </Link>
                        </li>
                      );
                    }
